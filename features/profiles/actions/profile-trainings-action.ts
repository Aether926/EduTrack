...
export async function getProfileTrainings(
    teacherId: string,
): Promise<TrainingRow[]> {
    ...
    const { data: attendanceRows, error: aErr } = await admin
        .from("Attendance")
        .select(
            "id, training_id, status, result, proof_url, proof_path, created_at, approved_hours",
        )
    ...
    const trainingIds = Array.from(
        new Set(attendance.map((r) => r.training_id)),
    );
    ...
    return attendance.map((a) => {
        ...
        return {
            ...
            approvedHours:
                a.approved_hours != null ? String(a.approved_hours) : null,
