...
export async function fetchEducationBackgroundToState(
    profileId: string,
): Promise<Partial<ProfileState>> {
...
export async function saveEducationBackgroundFromState(
    profileId: string,
    state: ProfileState,
): Promise<void> {
    const supabase = await createClient();
    const rows = mapStateToEducation(state, profileId).filter(
