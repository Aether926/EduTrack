...
export default function EmploymentInfoCard(props: {
  ...
  const { data, isEditing, onInputChange, onDateChange, viewerRole = "TEACHER" } = props;
  ...
  const { submitting, lastRequest, loadingLastRequest, hasPendingRequest, fetchLastRequest, submitRequest } =
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={data.position} onValueChange={(v) => onInputChange("position", v)}>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => onInputChange("plantillaNo", e.target.value)}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input
                  type="date"
                  value={data.dateOfOriginalAppointment ? data.dateOfOriginalAppointment.toISOString().split("T")[0] : ""}
                  onChange={(e) => onDateChange("dateOfOriginalAppointment", e.target.value ? new Date(e.target.value) : undefined)}
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">
                ...
                <Input
                  type="date"
                  value={data.dateOfLatestAppointment ? data.dateOfLatestAppointment.toISOString().split("T")[0] : ""}
                  onChange={(e) => onDateChange("dateOfLatestAppointment", e.target.value ? new Date(e.target.value) : undefined)}
                />
              </div>
            </div>
          </div>
      ...
      <Card className="flex flex-col border-0 shadow-lg">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {/* Only show on own profile page, never on public views */}
        ...
        <CardContent className="space-y-6">
          ...
          {isOwnProfile && props.from === "profile" && lastRequest && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_BADGE[lastRequest.status]}`}>
                {lastRequest.status}
              </span>
              {lastRequest.review_note && (
                <span className="text-gray-400 text-xs truncate">— {lastRequest.review_note}</span>
              )}
            </div>
          )}

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ReadOnlyField label="Employee ID" value={data.employeeId} icon={FileText} />
            <ReadOnlyField label="Position/Designation" value={data.position} icon={Briefcase} />
          </div>

          <ReadOnlyField label="Plantilla No." value={data.plantillaNo} icon={FileText} />
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <ReadOnlyDate label="Date of Original Appointment" value={data.dateOfOriginalAppointment} />
              <ReadOnlyDate label="Date of Latest Appointment" value={data.dateOfLatestAppointment} />
            </div>
          </div>
        </CardContent>
      </Card>
