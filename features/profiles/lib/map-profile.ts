...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function mapFamilyToState(family: any, children: any[]): Partial<ProfileState> {
...
export function mapStateToFamily(profile: ProfileState) {
  return {
    ...
    spouseNameExtension:
      profile.spouseNameExtension === "N/A" ? null : profile.spouseNameExtension || null,
    ...
    fatherNameExtension:
      profile.fatherNameExtension === "N/A" ? null : profile.fatherNameExtension || null,
...
export function mapStateToEducation(profile: ProfileState, profileId: string) {
  ...
  return levels.map(({ level, prefix }) => ({
    profileId,
    level,
    schoolName: (profile[`${prefix}School` as keyof ProfileState] as string) || null,
    degreeOrCourse: (profile[`${prefix}Degree` as keyof ProfileState] as string) || null,
    attendanceFrom: (profile[`${prefix}From` as keyof ProfileState] as string)
    ...
    attendanceTo: (profile[`${prefix}To` as keyof ProfileState] as string)
      ...
      : null,
    highestUnitsEarned: (profile[`${prefix}Units` as keyof ProfileState] as string) || null,
    yearGraduated: (profile[`${prefix}Graduated` as keyof ProfileState] as string)
      ? parseInt(profile[`${prefix}Graduated` as keyof ProfileState] as string)
      : null,
    scholarshipHonors: (profile[`${prefix}Honors` as keyof ProfileState] as string) || null,
  }));
}
