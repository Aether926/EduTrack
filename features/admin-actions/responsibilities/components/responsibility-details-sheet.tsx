...
import {
    ClipboardList, Pencil, X, Send, Loader2,
    BookOpen, Users, MoreHorizontal, Plus, Trash2,
} from "lucide-react";
...
type ScheduleBlock = { id: string; days: string[]; startTime: string; endTime: string };
...
function TimePicker({ value, onChange, error, placeholder }: {
    ...
    return (
        <div className={cn("flex items-center gap-2 rounded-md border bg-background px-3 py-2 w-full", error && "border-rose-500/50")}>
            <Select value={parsed.hour} onValueChange={(v) => update("hour", v)}>
                ...
                <SelectContent className="min-w-[4rem]">
                    {HOURS.map((h) => <SelectItem key={h.value} value={h.value} className="text-xs font-mono">{h.label}</SelectItem>)}
                </SelectContent>
            </Select>
            ...
            <Select value={parsed.minute} onValueChange={(v) => update("minute", v)}>
                ...
                <SelectContent className="min-w-[4rem]">
                    {MINUTES.map((m) => <SelectItem key={m.value} value={m.value} className="text-xs font-mono">{m.label}</SelectItem>)}
                </SelectContent>
            </Select>
            <div className="flex rounded border border-border/60 overflow-hidden ml-1">
                {["AM", "PM"].map((p) => (
                    <button key={p} type="button" onClick={() => update("ampm", p)}
                        className={cn("px-2 py-1 text-xs font-semibold transition-colors",
                            parsed.ampm === p ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}>
                        {p}
                    </button>
                ))}
            </div>
...
function DayPicker({ value, onChange, disabled }: { value: string[]; onChange: (d: string[]) => void; disabled?: boolean }) {
                ...
                return (
                    <button key={d.key} type="button" title={d.full} onClick={() => toggle(d.key)}
                        ...
                        className={cn(
                            ...
                            !disabled && !active && "hover:bg-muted/40 hover:text-foreground",
...
const TYPE_CONFIG: Record<string, { label: string; color: string; icon: React.ElementType }> = {
    TEACHING_LOAD: { label: "Teaching Load", color: "text-blue-400 border-blue-500/20 bg-blue-500/10", icon: BookOpen },
    COORDINATOR:   { label: "Coordinator Role", color: "text-emerald-400 border-emerald-500/20 bg-emerald-500/10", icon: Users },
    OTHER:         { label: "Other Duties", color: "text-orange-400 border-orange-500/20 bg-orange-500/10", icon: MoreHorizontal },
};
...
export function ResponsibilityDetailSheet({ open, onOpenChange, responsibility, onSuccess }: Props) {
    ...
    const typeConfig = TYPE_CONFIG[responsibility?.type ?? ""] ?? TYPE_CONFIG.OTHER;
    ...
    useEffect(() => {
        ...
        setSelectedTeacher(responsibility.teacher ? {
    ...
    const handleSave = async () => {
        ...
        const subjectValue = details.subject === "Other"
            ? details.customSubject ?? "" : details.subject ?? "";

        const cleanDetails = { ...details, subject: subjectValue, schedule: scheduleString || undefined, customSubject: undefined };
            ...
            <Sheet open={open} onOpenChange={handleClose}>
                ...
                >
                    ...
                    <SheetHeader className="px-5 py-4 border-b border-border/60 sticky top-0 bg-background z-10">
                        <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-2 min-w-0">
                                <div className={cn("rounded-lg border p-2 shrink-0", typeConfig.color)}>
                                    <TypeIcon className="h-4 w-4" />
                                </div>
                                <div className="min-w-0">
                                    <div className="flex flex-wrap items-center gap-2 mb-0.5">
                                        <span className={cn("inline-block rounded-full border px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wider", typeConfig.color)}>
                                            {typeConfig.label}
                                        </span>
                                        {isEdit && (
                                    ...
                                    <SheetTitle className="text-base leading-snug break-words whitespace-normal">
                                        {isEdit ? title || "Edit Responsibility" : responsibility.title}
                                    </SheetTitle>
                                </div>
                            </div>
                            <div className="flex items-center gap-1 shrink-0">
                                ...
                                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={handleClose} disabled={submitting}>
                                    <X className="h-4 w-4" />
                                </Button>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
                        ...
                        <div>
                            ...
                            {isEdit ? (
                                <Button
                                    variant="outline"
                                    className={cn("w-full justify-start font-normal", !selectedTeacher && "text-muted-foreground")}
                        ...
                        <div>
                            ...
                            {isEdit ? (
                                <Select value={type} onValueChange={(v) => { setType(v); setDetails({}); setScheduleBlocks([EMPTY_BLOCK()]); }}>
                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="TEACHING_LOAD"><div className="flex items-center gap-2"><BookOpen className="h-3.5 w-3.5 text-blue-400" />Teaching Load</div></SelectItem>
                                        <SelectItem value="COORDINATOR"><div className="flex items-center gap-2"><Users className="h-3.5 w-3.5 text-emerald-400" />Coordinator Role</div></SelectItem>
                                        <SelectItem value="OTHER"><div className="flex items-center gap-2"><MoreHorizontal className="h-3.5 w-3.5 text-orange-400" />Other Duties</div></SelectItem>
                                    </SelectContent>
                                </Select>
                            ) : (
                                <ReadOnlyField label="" value={typeConfig.label} />
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            {isEdit ? (
                                ...
                                <ReadOnlyField label="" value={responsibility.title} />
                            )}
                        </div>
                        ...
                        {(isEdit ? type : responsibility.type) === "TEACHING_LOAD" && (
                            <div className="space-y-4">
                                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Teaching Load Details</p>
                                ...
                                {isEdit ? (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            <Select value={details.subject ?? ""} onValueChange={(v) => { setDetail("subject", v); if (v !== "Other") setDetail("customSubject", ""); }}>
                                                <SelectTrigger><SelectValue placeholder="Select subject..." /></SelectTrigger>
                                                <SelectContent>{SUBJECTS.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                                            </Select>
                                            {details.subject === "Other" && (
                                                <Input className="mt-2" value={details.customSubject ?? ""} onChange={(e) => setDetail("customSubject", e.target.value)} placeholder="Enter subject name..." autoFocus />
                                            )}
                                        </div>
                                        {/* Grade + Section */}
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                <Select value={details.grade ?? ""} onValueChange={(v) => setDetail("grade", v)}>
                                            ...
                                            <div>
                                                ...
                                                <Input value={details.section ?? ""} onChange={(e) => setDetail("section", e.target.value)} placeholder="e.g. Rizal" />
                                            </div>
                                        </div>
                                        {/* Schedule blocks */}
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                ...
                                                {scheduleBlocks.length < 5 && (
                                                    <button type="button" onClick={() => setScheduleBlocks((p) => [...p, EMPTY_BLOCK()])}
                                            ...
                                            {scheduleBlocks.map((block, i) => {
                                                ...
                                                return (
                                                    <div key={block.id} className="rounded-lg border border-border/60 bg-background/60 p-3 space-y-3">
                                                        <div className="flex items-center justify-between">
                                                            <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Schedule {scheduleBlocks.length > 1 ? i + 1 : ""}</span>
                                                            {scheduleBlocks.length > 1 && (
                                                                <button type="button" onClick={() => setScheduleBlocks((p) => p.filter((b) => b.id !== block.id))} className="text-muted-foreground/50 hover:text-rose-400 transition-colors">
                                                        ...
                                                        <div>
                                                            ...
                                                            <DayPicker value={block.days} onChange={(days) => updateBlock(block.id, { ...block, days })} />
                                                        </div>
                                                        <div className="space-y-2">
                                                            ...
                                                            <div>
                                                                ...
                                                                <TimePicker value={block.startTime} onChange={(v) => updateBlock(block.id, { ...block, startTime: v })} placeholder="HH" />
                                                            </div>
                                                            <div>
                                                                <span className="text-[10px] text-muted-foreground/60 mb-1 block">End</span>
                                                                <TimePicker value={block.endTime} onChange={(v) => updateBlock(block.id, { ...block, endTime: v })} placeholder="HH" />
                                                            </div>
                                                        </div>
                                                        {preview && (
                                                            <div className="rounded-md bg-blue-500/5 border border-blue-500/20 px-3 py-2 text-xs text-blue-300/80">
                                                                <span className="font-medium text-blue-400/70">Preview: </span>{preview}
                                                            </div>
                                                        )}
                                                    </div>
                                                );
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <ReadOnlyField label="Subject" value={det.subject} />
                                        <ReadOnlyField label="Grade Level" value={det.grade} />
                                        <ReadOnlyField label="Section" value={det.section} />
                                        <ReadOnlyField label="Schedule" value={det.schedule} />
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {/* ── Coordinator details ── */}
                        {(isEdit ? type : responsibility.type) === "COORDINATOR" && (
                            <div className="space-y-4">
                                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Coordinator Details</p>
                                {isEdit ? (
                                    <div>
                                        <FieldLabel>Role</FieldLabel>
                                        <Input value={details.role ?? ""} onChange={(e) => setDetail("role", e.target.value)} placeholder="e.g. Department Head" />
                                    </div>
                                ) : (
                                    <ReadOnlyField label="Role" value={det.role} />
                                )}
                            </div>
                        )}
                        ...
                        {(isEdit ? type : responsibility.type) === "OTHER" && (
                            <div className="space-y-4">
                                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Other Duty Details</p>
                                {isEdit ? (
                                    <>
                                        <div>
                                            <FieldLabel optional>Organization / Committee</FieldLabel>
                                            <Input value={details.organization ?? ""} onChange={(e) => setDetail("organization", e.target.value)} placeholder="e.g. Math Club" />
                                        </div>
                                        <div>
                                    ...
                                    <>
                                        <ReadOnlyField label="Organization" value={det.organization} />
                                        <ReadOnlyField label="Description" value={det.description} />
                                    </>
                                )}
                            </div>
                        )}
                    ...
                    <div className="sticky bottom-0 bg-background border-t border-border/60 px-5 py-3 flex gap-2">
                        {isEdit ? (
                            <>
                                <Button variant="secondary" onClick={() => setMode("view")} disabled={submitting} className="flex-1">
                                    Cancel
                                </Button>
