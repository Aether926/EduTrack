...
import { ArrowUpDown, Plus, Search, X, ChevronDown, Download } from "lucide-react";
...
export function AppointmentHistoryClient(props: {
    ...
    const [selectedRow, setSelectedRow] = useState<AppointmentHistoryRow | null>(null);
    ...
    const hasFilters = typeFilter || positionFilter || subjectFilter || yearFilter || globalFilter;

    const filteredData = useMemo(() => {
        return (rows ?? [])
            .filter((r) => {
                if (typeFilter && r.appointment_type !== typeFilter) return false;
                if (positionFilter && r.position !== positionFilter) return false;
                if (subjectFilter && (r.teacher as any)?.subjectSpecialization !== subjectFilter) return false;
    ...
    const columns = useMemo<ColumnDef<any>[]>(
        () => [
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="flex items-center gap-3 min-w-0">
                            <InitialAvatar name={name} src={row.original.teacherProfileImage} className="h-8 w-8 shrink-0" />
                            <div className="min-w-0">
                                <div className="truncate text-sm font-medium">{name}</div>
                                <div className="truncate text-xs text-muted-foreground hidden md:block">{t?.email ?? "—"}</div>
                                <div className="md:hidden mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                    <Badge variant="outline" className={TYPE_STYLE[type] ?? "bg-muted text-muted-foreground border-border"}>{type}</Badge>
                                    <span className="truncate">{position || "—"}</span>
                                    <span className="font-mono">{fmt(start)}{end ? ` → ${fmt(end)}` : ""}</span>
                                </div>
                            </div>
                        </div>
                    );
            ...
            },
            {
                ...
                header: "Type",
                cell: ({ row }) => (
                    <Badge variant="outline" className={TYPE_STYLE[row.original.appointment_type] ?? "bg-muted text-muted-foreground border-border"}>
            ...
            },
            {
                ...
                header: "Start",
                cell: ({ row }) => <span className="text-sm text-muted-foreground font-mono">{fmt(row.original.start_date)}</span>,
            },
            {
                accessorKey: "end_date",
                header: "End",
                cell: ({ row }) => <span className="text-sm text-muted-foreground font-mono">{fmt(row.original.end_date)}</span>,
            },
            {
                accessorKey: "memo_no",
                header: "Memo No.",
                cell: ({ row }) => <span className="text-sm text-muted-foreground">{row.original.memo_no ?? "—"}</span>,
            },
            {
                accessorKey: "remarks",
                header: "Remarks",
                cell: ({ row }) => <span className="text-sm text-muted-foreground line-clamp-1">{row.original.remarks ?? "—"}</span>,
            },
        ],
        [],
    );
    ...
    const table = useReactTable({
        ...
        globalFilterFn: (row, _columnId, filterValue) => {
            ...
            return (
                ...
                String(row.original.teacher?.email ?? "").toLowerCase().includes(q) ||
                String(row.original.appointment_type ?? "").toLowerCase().includes(q) ||
                String(row.original.position ?? "").toLowerCase().includes(q) ||
                String(row.original.memo_no ?? "").toLowerCase().includes(q) ||
                String(row.original.remarks ?? "").toLowerCase().includes(q)
            );
        },
    ...
    return (
        <div className="space-y-4">
                ...
                <CardHeader className="gap-3 md:flex-row md:items-center md:justify-between border-b border-border/60 bg-gradient-to-br from-card to-background">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                        <div className="hidden md:block w-[280px]">
                            <Input value={globalFilter ?? ""} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Search teacher, type, memo..." />
                        </div>
                        ...
                        <div className="flex md:hidden items-center gap-2">
                            ...
                            <AnimatePresence initial={false}>
                                {searchOpen && (
                                    <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: "200px", opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="overflow-hidden">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="start">
                                ...
                                {APPOINTMENT_TYPES.map((t) => <DropdownMenuItem key={t} onClick={() => setTypeFilter(t)}>{t}</DropdownMenuItem>)}
                            </DropdownMenuContent>
                        </DropdownMenu>
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="outline" size="sm" className="gap-2 min-w-[110px] justify-between">
                                        <span className="truncate">{yearFilter ?? "All Years"}</span>
                                        <ChevronDown className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                                    </Button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="start" className="max-h-60 overflow-y-auto">
                                    <DropdownMenuItem onClick={() => setYearFilter(null)}>All Years</DropdownMenuItem>
                                ...
                                {table.getHeaderGroups().map((hg) => (
                                    <TableRow key={hg.id} className="hover:bg-transparent border-border/60">
                                        {hg.headers.map((header) => {
                                            ...
                                            const hide = ["appointment_type", "position", "start_date", "end_date", "memo_no", "remarks"].includes(id) ? "hidden md:table-cell" : "";
                                            return (
                                                <TableHead key={header.id} className={`${hide} bg-muted/30`}>
                                ...
                                {table.getRowModel().rows.length ? (
                                    table.getRowModel().rows.map((row, idx) => (
                                        <motion.tr
                                            ...
                                            onClick={() => {
                                                const orig = rows.find((r) => r.id === row.original.id) ?? null;
                                                setSelectedRow(orig);
                                                setDetailOpen(true);
                                            }}
                                        >
            ...
            <AddAppointmentSheet open={sheetOpen} onOpenChange={setSheetOpen} onSuccess={() => router.refresh()} />
