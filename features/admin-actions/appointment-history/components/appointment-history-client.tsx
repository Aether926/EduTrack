...
export function AppointmentHistoryClient(props: {
    ...
    () => [
          ...
          return t ? (
            <div className="min-w-0">
              ...
              <div className="md:hidden mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                <Badge variant="outline" className={TYPE_STYLE[type] ?? "bg-muted text-muted-foreground border-border"}>
                  {type}
                </Badge>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            variant="outline"
            className={TYPE_STYLE[row.original.appointment_type] ?? "bg-muted text-muted-foreground border-border"}
          >
      ...
      },
      {
        ...
        header: "Start",
        cell: ({ row }) => (
          <span className="text-sm text-muted-foreground">{fmt(row.original.start_date)}</span>
        ),
      },
      {
        ...
        header: "End",
        cell: ({ row }) => (
          <span className="text-sm text-muted-foreground">{fmt(row.original.end_date)}</span>
        ),
      },
      {
        accessorKey: "memo_no",
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const email = String(row.original.teacher?.email ?? "").toLowerCase();
      const type = String(row.original.appointment_type ?? "").toLowerCase();
