/* eslint-disable @typescript-eslint/no-explicit-any */
"use server";

import { createAdminClient } from "@/lib/supabase/server";

export async function upsertCompliancePolicy(form: {
  school_year: string;
  required_hours: number;
  at_risk_threshold_hours: number;
  period_start: string;
  period_end: string;
  is_edit?: boolean;
}) {
  const admin = createAdminClient();

  const { data: existing } = await admin
    .from("TrainingCompliancePolicy")
    .select("id")
    .eq("school_year", form.school_year)
    .is("school_id", null)
    .single();

  if (existing?.id) {
    const updatePayload = form.is_edit
      ? {
          at_risk_threshold_hours: form.at_risk_threshold_hours,
          period_start: form.period_start,
          period_end: form.period_end,
          updated_at: new Date().toISOString(),
        }
      : {
          required_hours: form.required_hours,
          at_risk_threshold_hours: form.at_risk_threshold_hours,
          period_start: form.period_start,
          period_end: form.period_end,
          updated_at: new Date().toISOString(),
        };

    const { error } = await admin
      .from("TrainingCompliancePolicy")
      .update(updatePayload)
      .eq("id", existing.id);

    if (error) throw new Error(error.message);
  } else {
    const { is_edit: _removed, ...insertData } = form;
    const { error } = await admin
      .from("TrainingCompliancePolicy")
      .insert({ ...insertData, school_id: null });

    if (error) throw new Error(error.message);
  }

  await recomputeAllCompliance();
}

export async function recomputeAllCompliance() {
  const admin = createAdminClient();

  const month = new Date().getMonth() + 1;
  const year = new Date().getFullYear();
  const schoolYear =
    month >= 6 ? `SY ${year}-${year + 1}` : `SY ${year - 1}-${year}`;

  const { data: teachers } = await admin
    .from("User")
    .select("id")
    .eq("role", "TEACHER")
    .eq("status", "APPROVED");

  if (!teachers?.length) return;

  await Promise.all(
    teachers.map((t: any) =>
      admin.rpc("compute_teacher_training_compliance", {
        p_teacher_id: t.id,
        p_school_year: schoolYear,
      }),
    ),
  );
}