...
async function requireAdmin() {
  ...
  if (user?.role !== "ADMIN") return { ok: false as const, error: "Unauthorized", userId: auth.user.id };
...
// ── Fetch all teacher doc statuses ────────────────────────────────────────────
...
export async function getAllTeacherDocumentStatus() {
  ...
  return teacherIds.map((tid) => {
    ...
    return {
      ...
      missing:   required.filter((dt) => !docs.find((d) => d.document_type_id === dt.id)).length,
...
export async function getPendingDocumentRequests() {
  ...
  try {
    ...
    const teacherIds = [
      ...new Set((requests ?? []).map((r: any) => r.teacher_id).filter(Boolean)),
    ];

    const [{ data: profiles, error: pErr }, { data: hrs, error: hErr }] =
      await Promise.all([
        admin
          .from("Profile")
...
export async function approveTeacherDocument(docId: string): Promise<ActionResult> {
  try {
    ...
    const { data: doc } = await admin
      .from("TeacherDocument")
      .select("teacher_id, document_type_id, DocumentType:document_type_id(name)")
      .eq("id", docId).single();
    if (!doc) return { ok: false, error: "Document not found" };

    const { error } = await admin.from("TeacherDocument").update({
      status: "APPROVED", reviewed_at: now, reviewed_by: adminCheck.userId, reject_reason: null,
    }).eq("id", docId);
    ...
    await insertActivity([{
      ...
      meta: { docId, documentTypeId: doc.document_type_id, docTypeName },
    }]);
...
export async function rejectTeacherDocument(docId: string, reason: string): Promise<ActionResult> {
  try {
    ...
    const { error } = await admin.from("TeacherDocument").update({
      status: "REJECTED", reviewed_at: now, reviewed_by: adminCheck.userId, reject_reason: reason.trim(),
    }).eq("id", docId);
    if (error) return { ok: false, error: error.message };
    ...
    await insertActivity([{
      ...
      meta: { docId, documentTypeId: doc.document_type_id, docTypeName, reason: reason.trim() },
    }]);
...
// ── Admin: Direct delete document (with bucket cleanup) ───────────────────────
...
export async function adminDeleteDocument(docId: string): Promise<ActionResult> {
  try {
    ...
    const { data: doc } = await admin
      .from("TeacherDocument")
      .select("teacher_id, file_path, document_type_id, DocumentType:document_type_id(name)")
      .eq("id", docId).single();
    if (!doc) return { ok: false, error: "Document not found" };

    // delete file from bucket first
    if (doc.file_path)
      await admin.storage.from("teacher-documents").remove([doc.file_path]);

    // close any pending requests for this doc
    await admin.from("DocumentRequest")
      .update({ status: "REJECTED", reviewed_at: now, reviewed_by: adminCheck.userId, admin_note: "Document was deleted by admin." })
...
// ── Admin: Direct resubmit request to teacher ────────────────────────────────
...
export async function adminRequestResubmit(
  ...
  try {
    ...
    // check no pending resubmit already
    const { data: existing } = await admin
      .from("DocumentRequest").select("id")
      .eq("teacher_document_id", docId)
      .eq("type", "RESUBMIT").eq("status", "PENDING").maybeSingle();
    if (existing) return { ok: false, error: "A resubmit request is already pending for this document." };

    // mark doc as REJECTED so teacher can resubmit
    await admin.from("TeacherDocument").update({
      status: "REJECTED",
      reviewed_at: now,
      reviewed_by: adminCheck.userId,
      reject_reason: adminNote.trim() || "Admin has requested a resubmission.",
    }).eq("id", docId);

    // create a request so it shows in teacher's pending list
...
// ── Admin: Approve document request (RESUBMIT or DELETE) ─────────────────────
...
export async function approveDocumentRequest(requestId: string): Promise<ActionResult> {
  try {
    ...
    const { data: req } = await admin
      .from("DocumentRequest")
      .select("*, TeacherDocument:teacher_document_id(*), DocumentType:document_type_id(*)")
    ...
    if (req.type === "DELETE") {
      ...
      if (filePath)
        await admin.storage.from("teacher-documents").remove([filePath]);
      ...
      await admin.from("TeacherDocument").update({
        ...
        reviewed_at: now,
        reviewed_by: adminCheck.userId,
        reject_reason: "Resubmission approved — please upload a new document.",
      }).eq("id", req.teacher_document_id);
    }

    const { error } = await admin.from("DocumentRequest").update({
      status: "APPROVED",
      reviewed_by: adminCheck.userId,
      reviewed_at: now,
    }).eq("id", requestId);
...
export async function rejectDocumentRequest(
  ...
  try {
    ...
    await insertActivity([{
      ...
      entity_id: requestId,
      message,
      meta: { requestId, docTypeName, type: req.type, adminNote: adminNote.trim() },
    }]);
