...
export async function getMyDocumentChecklist() {
  ...
  const [{ data: docTypes }, { data: submissions }, { data: requests }] = await Promise.all([
    admin.from("DocumentType").select("*").order("required", { ascending: false }),
    admin.from("TeacherDocument").select("*").eq("teacher_id", auth.user.id),
    admin.from("DocumentRequest")
      .select("*")
      .eq("teacher_id", auth.user.id)
      .eq("status", "PENDING"),
  ]);

  const submissionMap = new Map((submissions ?? []).map((s) => [s.document_type_id, s]));
  const requestMap    = new Map((requests ?? []).map((r) => [r.teacher_document_id, r]));

  return (docTypes ?? []).map((dt) => {
    const submission     = submissionMap.get(dt.id) ?? null;
    const pendingRequest = submission ? (requestMap.get(submission.id) ?? null) : null;
...
export async function getDocumentSignedUrl(
  ...
  try {
    ...
    if (signErr || !signed?.signedUrl)
      return { ok: false, error: signErr?.message ?? "Failed to generate URL" };
...
export async function submitTeacherDocument(
  ...
  try {
    ...
    const { data: docType } = await admin
      .from("DocumentType").select("*").eq("id", documentTypeId).single();
    if (!docType) return { ok: false, error: "Document type not found" };

    if (docType.allowed_mime && !docType.allowed_mime.includes(file.type))
      return { ok: false, error: `Invalid file type. Allowed: ${docType.allowed_mime.join(", ")}` };
    if (docType.max_mb && file.size > docType.max_mb * 1024 * 1024)
      return { ok: false, error: `File too large. Maximum: ${docType.max_mb}MB` };
    ...
    if (existing?.file_path)
      await admin.storage.from("teacher-documents").remove([existing.file_path]);
    ...
    const { error: uploadErr } = await admin.storage
      .from("teacher-documents").upload(path, file, { upsert: true });
    ...
    await insertActivity([{
      ...
      meta: { documentTypeId, docTypeName: docType.name, docTypeCode: docType.code },
    }]);
...
export async function requestResubmit(
  ...
  try {
    ...
    if (doc.teacher_id !== auth.user.id) return { ok: false, error: "Unauthorized" };

    const { data: existing } = await admin
      .from("DocumentRequest").select("id")
      .eq("teacher_document_id", docId)
      .eq("type", "RESUBMIT").eq("status", "PENDING").maybeSingle();
    if (existing) return { ok: false, error: "You already have a pending resubmit request." };

    const { data: req, error } = await admin.from("DocumentRequest").insert({
      teacher_id: auth.user.id,
...
export async function requestDelete(
  ...
  try {
    ...
    const { data: existing } = await admin
      .from("DocumentRequest").select("id")
      .eq("teacher_document_id", docId)
      .eq("type", "DELETE").eq("status", "PENDING").maybeSingle();
    if (existing) return { ok: false, error: "You already have a pending delete request." };
