...
import { Hourglass, XCircle, ShieldX, LogOut, Loader2 } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      const { data: urow, error } = await supabase
        .from("User")
        .select("status, suspensionReason")
        .eq("id", user.id)
        .maybeSingle();
      ...
      // Store suspension reason if suspended
      if (urow.status === "SUSPENDED") {
        setSuspensionReason(urow.suspensionReason ?? null);
      }
        ...
        <div className="relative w-full max-w-sm" ref={cardRef}>
          ...
          <div className="relative bg-[#141415] border border-[#2a2a2d] rounded-2xl overflow-hidden shadow-xl">
            <div className="p-8 text-center">
              ...
              {isRejected && (
                <>
                  ...
                  <div className="mt-6 text-[12px] text-[#555560]">
                    Status: <span className="text-red-400 font-medium">REJECTED</span>
                  </div>
                </>
              )}
              ...
              {isSuspended && (
                <>
                  <div className="mx-auto mb-4 w-12 h-12 rounded-full bg-[#1c1c1e] border border-orange-500/20 flex items-center justify-center">
                    <ShieldX className="h-6 w-6 text-orange-400" />
                  </div>
                  <h1 className="text-[20px] font-bold text-[#f0f0f0] mb-2">
                    Account Suspended
                  </h1>
                  ...
                  {/* Suspension reason */}
                  {suspensionReason && (
                    <div className="mt-4 px-4 py-3 rounded-lg border border-orange-500/20 bg-orange-500/10 text-left">
                  ...
                  <button
                    onClick={() =>
                      supabase.auth.signOut({ scope: "local" }).then(() => router.push("/signin"))
                    }
                  ...
                  <div className="mt-4 text-[12px] text-[#555560]">
                    Status: <span className="text-orange-400 font-medium">SUSPENDED</span>
                  </div>
                </>
              )}
              ...
              {!isRejected && !isSuspended && (
                <>
                  ...
                  <div className="mt-4 text-[12px] text-[#555560]">
                    Status:{" "}
                    <span className="text-[#f0f0f0] font-medium">{statusParam}</span>
                  </div>
                </>
              )}
