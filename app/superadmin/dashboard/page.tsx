...
export default async function SuperadminDashboardPage() {
    ...
    const [
        ...
        admin.from("User").select("*", { count: "exact", head: true }).eq("role", "TEACHER").eq("status", "APPROVED"),
        admin.from("User").select("*", { count: "exact", head: true }).eq("role", "ADMIN").eq("status", "APPROVED"),
        admin.from("User").select("*", { count: "exact", head: true }).eq("status", "PENDING"),
        admin.from("User").select("*", { count: "exact", head: true }).eq("status", "SUSPENDED"),
        admin.from("User").select("*", { count: "exact", head: true }).eq("role", "SUPERADMIN"),
        fetchSecurityLogs(),
    ]);
