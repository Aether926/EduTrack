...
export async function GET(req: NextRequest) {
    ...
    const C = {
        ...
        textMid: "FF444444",
        textMuted: "FF888888",
        compliantBg: "FFE6F4EA",
        compliantFg: "FF1E6B2E",
        atRiskBg: "FFFEF3CD",
        atRiskFg: "FF7D5A00",
        nonCompBg: "FFFCE8E8",
        nonCompFg: "FF8B0000",
    ...
    const statusStyle = (status: string) => {
        if (status === "COMPLIANT")
            return { bg: C.compliantBg, fg: C.compliantFg, label: "Compliant" };
        if (status === "AT_RISK")
            return { bg: C.atRiskBg, fg: C.atRiskFg, label: "At Risk" };
    ...
    const buildLetterhead = (
        ...
        for (let col = 1; col <= colCount; col++)
            sheet.getCell(4, col).fill = solid(C.headerBg);
        ...
        for (let col = 1; col <= colCount; col++)
            sheet.getCell(6, col).fill = solid(C.headerBg);
        ...
        c(7).value =
            `School Year: ${schoolYear}     |     Date Generated: ${dateStr}`;
        ...
        for (let col = 1; col <= colCount; col++) {
            sheet.getCell(1, col).border = {
                top: { style: "thin", color: { argb: C.borderMed } },
            };
            sheet.getCell(7, col).border = {
                bottom: { style: "thin", color: { argb: C.borderMed } },
            };
        }
        ...
        for (let row = 1; row <= 7; row++) {
            sheet.getCell(row, 1).border = {
                ...sheet.getCell(row, 1).border,
                left: { style: "thin", color: { argb: C.borderMed } },
            };
            sheet.getCell(row, colCount).border = {
                ...sheet.getCell(row, colCount).border,
                right: { style: "thin", color: { argb: C.borderMed } },
            };
        }
    };
    ...
    if (type === "admin") {
        ...
        const [{ data: profiles }, { data: profileHRs }] =
            await (teacherIds.length
                ? Promise.all([
                      admin
                          .from("Profile")
        ...
        setTotalCell(
            "H",
            `${nC} Compliant   ${nA} At Risk   ${nN} Non-Compliant`,
        );
        ...
        const { data: attendance } = await admin
            .from("Attendance")
            .select(
                "id, approved_hours, status, result, training_id, ProfessionalDevelopment(title, type, start_date, end_date, total_hours, sponsoring_agency)",
            )
            .eq("teacher_id", teacherId)
            .eq("status", "APPROVED")
            .eq("result", "PASSED");
        ...
        const infoRows: [string, string][] = [
            ...
            [
                "Period:",
                policy
                    ? `${policy.period_start}  to  ${policy.period_end}`
                    : "—",
            ],
        ];
        ...
        infoRows.forEach(([label, value], i) => {
            ...
            for (let col = 1; col <= 6; col++)
                sheet.getCell(r, col).fill = solid(bg);
        ...
        buildSectionTitle(
            sheet,
            TRAIN_TITLE,
            "TRAININGS COUNTED TOWARD COMPLIANCE",
            "F",
        );
        ...
        const totalHrs = countedTrainings.reduce(
            (s, t) =>
                s +
                (t.approved_hours ??
                    (t.ProfessionalDevelopment as any)?.total_hours ??
                    0),
            0,
        );
