...
export async function GET(req: NextRequest) {
    ...
    const [
        ...
        admin
            .from("Profile")
            .select(
                "firstName, middleInitial, lastName, dateOfBirth, gender, civilStatus, nationality, address, email, contactNumber, pagibigNo, philHealthNo, gsisNo, tinNo, bachelorsDegree, postGraduate, subjectSpecialization",
            )
            .eq("id", teacherId)
            .single(),
        admin
            .from("ProfileHR")
            .select(
                "employeeId, plantillaNo, position, dateOfOriginalAppointment, dateOfLatestAppointment",
            )
            .eq("id", teacherId)
            .single(),
        admin
            ...
            .select(
                "id, appointment_type, position, start_date, end_date, memo_no, remarks",
            )
    ...
    const { data: pdRows } = trainingIds.length
        ? await admin
              ...
              .select(
                  "id, title, type, start_date, end_date, total_hours, sponsoring_agency",
              )
              .in("id", trainingIds)
        : { data: [] };
    ...
    const blob = await pdf(
        React.createElement(ServiceRecordDocument, {
            data,
