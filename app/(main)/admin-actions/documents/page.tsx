    ...
    return (
        <div className="mx-auto w-full max-w-7xl px-4 py-5 md:px-6 md:py-6 space-y-6">
                ...
                <div className="relative px-5 py-5 md:px-6 md:py-6">
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div className="flex items-center gap-3 md:flex-1">
                            ...
                            <div>
                                <h1 className="text-lg font-semibold tracking-tight leading-tight">Teacher Documents</h1>
                                <p className="text-[13px] text-muted-foreground mt-0.5">Review and manage 201 file submissions</p>
                            </div>
                        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
                {[
                    { label: "Approved",  value: totalApproved,  icon: <CheckCircle2 className="h-5 w-5 text-emerald-400" />, color: "border-emerald-500/20 bg-emerald-500/5", valueClass: "text-emerald-400" },
                    { label: "Pending",   labelFull: "Pending Review", value: totalSubmitted, icon: <Clock className="h-5 w-5 text-blue-400" />, color: "border-blue-500/20 bg-blue-500/5", valueClass: "text-blue-400" },
                    { label: "Rejected",  value: totalRejected,  icon: <XCircle className="h-5 w-5 text-rose-400" />,    color: "border-rose-500/20 bg-rose-500/5",    valueClass: "text-rose-400"    },
                    { label: "Missing",   value: totalMissing,   icon: <AlertCircle className="h-5 w-5 text-amber-400" />, color: "border-amber-500/20 bg-amber-500/5", valueClass: "text-amber-400"  },
                ].map((s) => (
                        ...
                        <CardContent className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground mt-1 whitespace-nowrap">
                                    ...
                                    <span className="hidden sm:inline">{(s as any).labelFull ?? s.label}</span>
                                </p>
                            </div>
                        </CardContent>
                    </Card>
            ...
            <div>
                <h2 className="text-lg font-semibold mb-3">Pending Review ({pendingDocs.length})</h2>
                <AdminDocumentReviewTable docs={pendingDocs as any} requests={(requests ?? []) as any} />
            </div>
            <div>
