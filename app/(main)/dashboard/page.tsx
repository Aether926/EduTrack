...
export default async function DashboardPage() {
    ...
    const [stats, events, eligibility, adminStats, adminEvents] =
        await Promise.all([
    ...
    const q = db
        .from("ActivityLog")
        .select(
            "id, created_at, action, message, meta, target_user_id, actor_id, entity_type, entity_id, recipient_role",
        )
    ...
    if (isAdmin) {
        ...
        q.eq("actor_id", user.id).eq("recipient_role", "actor");
    } else {
        // Teacher sees rows targeted at them
        q.eq("target_user_id", user.id);
    }
