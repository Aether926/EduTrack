...
function LevelPill({ level }: { level: string }) {
    ...
    const cls =
        l === "regional"        ? "bg-amber-500/10 text-amber-400 border-amber-500/40"
...
export default function AddTrainingAndSeminar({ data }: AddTrainingAndSeminarProps) {
    ...
    const [selected, setSelected] = useState<TrainingSeminarTableRow | null>(null);
    ...
    const [statusFilter, setStatusFilter] = useState<"upcoming" | "past" | null>(null);
    ...
    const emptyForm = {
        title: "", type: "TRAINING" as "TRAINING" | "SEMINAR",
        level: "REGIONAL" as "REGIONAL" | "NATIONAL" | "INTERNATIONAL",
        sponsoring_agency: "", total_hours: "",
    ...
    const fillForm = (pd: ProfessionalDevelopment) => setFormData({
        title: pd.title ?? "",
    ...
    const openCreate = () => { setMode("create"); setSelected(null); setFormData(emptyForm); setIsOpen(true); };
    ...
    const handleAssign = () => {
        if (selected && !isPastDate(selected.raw?.start_date)) {
            ...
            toast.warning("This training has already passed. You can no longer assign teachers to it.");
        }
    };

    const triggerDelete = () => {
        if (!selected) return;
        setPendingDelete({ kind: "single", id: selected.id, title: selected.title, type: String(selected.type) });
        setDeleteDialogOpen(true);
    };

    const openBulkDelete = (rows: TrainingSeminarTableRow[]) => {
        setPendingDelete({ kind: "bulk", ids: rows.map(r => r.id), count: rows.length });
        setDeleteDialogOpen(true);
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (result.success) {
                toast.success(pendingDelete.kind === "single" ? "Deleted successfully." : `Deleted ${result.count} item(s).`);
    ...
    const filteredData = useMemo(() => {
        return data.filter((row) => {
            ...
            if (statusFilter) {
                const start = row.raw?.start_date ? new Date(row.raw.start_date) : null;
                if (statusFilter === "upcoming" && start && start < today) return false;
                if (statusFilter === "past" && start && start >= today) return false;
            }
            return true;
        });
    ...
    const columns = useMemo<ColumnDef<TrainingSeminarTableRow>[]>(
        () => [
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="min-w-0 max-w-[180px] sm:max-w-[300px]">
                            ...
                            {past && (
                                <div className="text-[10px] text-muted-foreground/50 mt-0.5">Expired</div>
                            )}
                        </div>
                    );
                },
            ...
            },
            {
                ...
                header: "Date",
                cell: ({ row }) => (
                    <div className="font-mono text-xs text-muted-foreground">{row.getValue("date")}</div>
                ),
            },
            {
        ...
        <div className="rounded-xl border border-border/60 overflow-hidden">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="sm" className="gap-2 min-w-[130px] justify-between bg-background/50">
                                    <span className="truncate">{typeFilter ? typeFilter.charAt(0) + typeFilter.slice(1).toLowerCase() : "All Types"}</span>
                                    <ChevronDown className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="start">
                                <DropdownMenuItem onClick={() => setTypeFilter(null)}>All Types</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setTypeFilter("TRAINING")}>Training</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setTypeFilter("SEMINAR")}>Seminar</DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="sm" className="gap-2 min-w-[130px] justify-between bg-background/50">
                                    <span className="truncate">{levelFilter ? levelFilter.charAt(0) + levelFilter.slice(1).toLowerCase() : "All Levels"}</span>
                                    <ChevronDown className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="start">
                                <DropdownMenuItem onClick={() => setLevelFilter(null)}>All Levels</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setLevelFilter("REGIONAL")}>Regional</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setLevelFilter("NATIONAL")}>National</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setLevelFilter("INTERNATIONAL")}>International</DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                            ...
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="sm" className="gap-2 min-w-[130px] justify-between bg-background/50">
                                    <span className="truncate">
                                        {statusFilter === "upcoming" ? "Upcoming" : statusFilter === "past" ? "Past" : "All Statuses"}
                                    </span>
                                    <ChevronDown className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                                </Button>
                            </DropdownMenuTrigger>
                        ...
                        {hasFilters && (
                            <Button variant="ghost" size="sm" onClick={() => { setTypeFilter(null); setLevelFilter(null); setStatusFilter(null); }} className="text-muted-foreground gap-1">
                    ...
                    {selectedRows.length > 0 && (
                        <div className="flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-between rounded-lg border border-rose-500/20 bg-rose-500/5 px-4 py-2.5">
                            ...
                            <div className="flex gap-2">
                                ...
                                <Button size="sm" className="gap-2 bg-rose-600 hover:bg-rose-700 text-white" onClick={() => openBulkDelete(selectedRows)}>
                ...
                <Table className="min-w-[600px]">
                    <TableHeader>
                        {table.getHeaderGroups().map((hg) => (
                            <TableRow key={hg.id} className="bg-card border-border/60 hover:bg-transparent">
                                {hg.headers.map((header) => {
                                    const id = header.column.id;
                                    const hide = id === "select" || id === "date" || id === "sponsor" ? "hidden md:table-cell" : "";
                                    return (
                                        <TableHead key={header.id} className={`${hide} text-muted-foreground/70 text-xs uppercase tracking-wider`}>
                    ...
                    <TableBody>
                        {table.getRowModel().rows.length ? (
                                ...
                                return (
                                    ...
                                    >
                                        {row.getVisibleCells().map((cell) => {
                                            ...
                                            const hide = id === "select" || id === "date" || id === "sponsor" ? "hidden md:table-cell" : "";
                                            return (
                                                <TableCell key={cell.id} className={hide}>
                            ...
                            <TableRow>
                                <TableCell colSpan={columns.length} className="bg-card h-24 text-center text-muted-foreground">
                                    No results.
                                </TableCell>
                            </TableRow>
                        )}
            ...
            <div className="bg-card flex flex-wrap items-center justify-between gap-2 px-5 py-3.5 border-t border-border/60">
                <div className="text-xs text-muted-foreground">
                    {filteredCount} result{filteredCount === 1 ? "" : "s"} • Page {table.getState().pagination.pageIndex + 1} of {pageCount || 1}
                </div>
                <div className="flex gap-2">
            ...
            {/* ── Delete dialog ── */}
            <AlertDialog open={deleteDialogOpen} onOpenChange={(open) => { setDeleteDialogOpen(open); if (!open) setPendingDelete(null); }}>
                <AlertDialogContent>
                    <AlertDialogHeader>
